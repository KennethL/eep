...
This EEP introduces the `maybe ... end` expression to make it a construct
usable for control flow and value-based error handling based on pattern
matching.

This introduces `maybe ... end` along with a new contextual use
...
We propose the `maybe ... end` construct which is similar to `begin ... end` in that it is used to group multiple distinct expression as a single block. But there is one important difference in that the `maybe` block does not export its variables while `begin` indeed does export its variables.
...
**FIXME** **the variable `DispatchData` introduced below does not make sense since there is no export of variables in `maybe` blocks.
I propose that we remove this example completely?**
...
The proposed solution is that if the `maybe` block is terminated before all expressions executed successfully and there is no `else`clause an exception is generated.
An alternative solution would be to make the `else` clause mandatory. 
...
Since we are now planning to introduce a new mechanism for activation of new language features per module the potential risk for incompatibility with introducing a new keyword is reduced. Only modules explicitly using the new language feature will be impacted.
...
- `=~` could make sense. It is a new operator that does not clash with anything existing and it could be thought of as approximately matching.

The `<-` or the `=~` operators makes the most sense but we choose `=~`
since `<-` in its current use means *sub set of* or *member of* which is not really what it is used for here.
